...
```
After this the game queries for 5 random events (As seen in [here](https://github.com/margacampos/MongoDB-hackathon-2022/blob/main/pages/api/events-and-articles-today.ts)), which are saved for later use, and one is selected as the winner event.
#### Dialogs:
...
The archive has three inputs: country, actor and event. They work as a select element and each time one of your selections changes, the [database is queried](https://github.com/margacampos/MongoDB-hackathon-2022/blob/main/pages/api/archive-events.ts) returning the events displayed in screen.

The way events are displayed is through a svg that makes it seem like you are looking for the events in a file cabinet. You can open it and close it and click on the events it contains to open a pop up with more information about them such as the external link to read the full article.

You can use the arrows above to see more results to your query.

### Achievements:
You can earn badges while looking around the website. Right now, there are five of them. If you visit the [gallery](https://mongo-db-hackathon-2022.vercel.app/gallery), there is a section about them.

There are two functions that check for achievements. One is in the "Enter the newsroom section" and the other one is in the archives. Once they find you have done something worth a badge, they update the achievements array in gameObject with that achievement's id. Then, a [notification](https://github.com/margacampos/MongoDB-hackathon-2022/blob/main/components/Game/AchievePopUp.tsx) appears and the [Achievements](https://github.com/margacampos/MongoDB-hackathon-2022/blob/main/components/Game/Achievements.tsx) component renders the badges when clicked on the bottom left badges button.
